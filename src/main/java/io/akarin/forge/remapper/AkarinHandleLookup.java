...
public class AkarinHandleLookup {
    ...
    public static MethodHandle unreflect(MethodHandles.Lookup lookup, Method m2) throws IllegalAccessException {
        if (m2.getDeclaringClass().getName().equals("java.lang.Class")) {
            switch (m2.getName()) {
                case "forName": {
                    return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), String.class);
                }
                case "getField": 
                case "getDeclaredField": {
                    return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), Class.class, String.class);
                }
                case "getMethod": 
                case "getDeclaredMethod": {
                    return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), Class.class, String.class, Class[].class);
                }
                case "getSimpleName": {
                    return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), Class.class);
                }
            }
        } else if (m2.getName().equals("getName")) {
            if (m2.getDeclaringClass().getName().equals("java.lang.reflect.Field")) {
                return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), Field.class);
            }
            if (m2.getDeclaringClass().getName().equals("java.lang.reflect.Method")) {
                return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), Method.class);
            }
        } else if (m2.getName().equals("loadClass") && m2.getDeclaringClass().getName().equals("java.lang.ClassLoader")) {
            return AkarinHandleLookup.getClassReflectionMethod(lookup, m2.getName(), ClassLoader.class, String.class);
        }
    ...
    static {
        try {
            AkarinHandleLookup.loadMappings(new BufferedReader(new InputStreamReader(MappingLoader.class.getClassLoader().getResourceAsStream("mappings/" + AkarinForge.getNativeVersion() + "/cb2srg.srg"))));
        }
