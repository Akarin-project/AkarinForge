...
public class Remaps {
    ...
    public static final String NMS_VERSION = Constants.NMS_VERSION;

    public static String reverseMapExternal(Class<?> name) {
        return Remaps.reverseMap(name).replace('$', '.').replace('/', '.');
    }

    public static String reverseMap(Class<?> name) {
        return Remaps.reverseMap(Type.getInternalName(name));
    }
    ...
    public static String mapMethod(Class<?> inst, String name, Class<?> ... parameterTypes) {
        String result = Remaps.mapMethodInternal(inst, name, parameterTypes);
        if (result != null) {
            return result;
        }
        return name;
    ...
    public static String mapMethodInternal(Class<?> inst, String name, Class<?> ... parameterTypes) {
        ...
        String match = Remaps.reverseMap(inst) + "/" + name;
        ...
        for (String value : colls) {
            ...
            for (Type type : Type.getArgumentTypes((String)str[1])) {
                ...
                if (i2 >= parameterTypes.length || !typename.equals(Remaps.reverseMapExternal(parameterTypes[i2]))) {
                    i2 = -1;
                    break;
                }
                ++i2;
        ...
        if (superClass != null && (superMethodName = Remaps.mapMethodInternal(superClass, name, parameterTypes)) != null) {
            return superMethodName;
        }
        for (Class<?> interfaceClass : inst.getInterfaces()) {
            String superMethodName2 = Remaps.mapMethodInternal(interfaceClass, name, parameterTypes);
    ...
    public static String mapFieldName(Class<?> inst, String name) {
        Class<?> superClass;
        String key = Remaps.reverseMap(inst) + "/" + name;
    ...
    public static String mapClass(String className) {
        String tRemapped = JarRemapper.mapTypeName(className, ReflectionTransformer.jarMapping.packages, ReflectionTransformer.jarMapping.classes, (String)className);
        if (tRemapped.equals(className) && className.startsWith("net/minecraft/server/") && !className.contains(NMS_VERSION)) {
            ...
            return JarRemapper.mapTypeName(tNewClassStr, ReflectionTransformer.jarMapping.packages, ReflectionTransformer.jarMapping.classes, (String)className);
        }
        return tRemapped;
    }
    ...
    public static String demapFieldName(Field field) {
        ...
        String match = Remaps.reverseMap(field.getDeclaringClass()) + "/";
    ...
    public static String demapMethodName(Method method) {
        ...
        String match = Remaps.reverseMap(method.getDeclaringClass()) + "/";
        ...
        while (clazz != null && clazz.getClassLoader() != null) {
            ...
            if (checkSuperClass) {
                for (Class<?> interfaceClass : clazz.getInterfaces()) {
                    if (!Remaps.isClassNeedRemap(interfaceClass, true)) continue;
                    return true;
                }
