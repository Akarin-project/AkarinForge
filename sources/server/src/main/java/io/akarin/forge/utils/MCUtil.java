...
public final class MCUtil {
    ...
    public static <T> T ensureMain(String reason, Supplier<T> run) {
        if (Thread.currentThread() != MinecraftServer.instance().primaryThread) {
            ...
            MinecraftServer.instance().processQueue.add(wait);
            try {
                return wait.get();
    ...
    public static TileEntityHopper getHopper(World world, BlockPos pos) {
        ...
        if (chunk != null && chunk.getBlockState(pos).getBlock() == Blocks.HOPPER) {
            TileEntity tileEntity = chunk.getTileEntity(pos, EnumCreateEntityType.IMMEDIATE);
