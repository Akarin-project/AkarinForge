...
public class GameData
{
    ...
    {
        FMLLog.log.debug("Injecting existing registry data into this {} instance", FMLCommonHandler.instance().getEffectiveSide().isServer() ? "server" : "client"); // Akarin - down to debug
    ...
    public static ResourceLocation checkPrefix(String name, boolean warnOverrides)
    {
        ...
        {
            FMLLog.log.debug("Potentially Dangerous alternative prefix `{}` for name `{}`, expected `{}`. This could be a intended override, but in most cases indicates a broken mod.", oldPrefix, name, prefix); // Akarin - down to debug
            prefix = oldPrefix;
        }
